...
Mode configuration arguments are generic for all modes, and correspond to the flat buffer mode configuration table names:
...
Other hardware can be configured at the same time as the mode change, for example power supply, and pull-up resistors. The parameters are also identical to the flat buffer configuration request table names:
